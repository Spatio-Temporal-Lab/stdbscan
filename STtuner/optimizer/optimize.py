...
from optimizer.utils import HistoryContainer

from optimizer.utils import impute_incumb_values, max_min_distance
from optimizer.utils import Observation
from autotune.pipleline.base import BOBase
from optimizer.utils import MAXINT, SUCCESS, FAILED
from optimizer.utils import estimate_size, get_space_feature
from autotune.selector.selector import KnobSelector
from optimizer.transfer.tlbo import RGPE
from optimizer.utils import check_random_state
from optimizer.utils import ConfigurationSpace, UniformIntegerHyperparameter, CategoricalHyperparameter

from autotune.knobs import logger
from optimizer.smac import BO_Optimizer


class Optimizer(BOBase):
    ...
    def __init__(self, objective_function: callable,
        ...
        super().__init__(objective_function, config_space, task_id=task_id, logging_dir=logging_dir,
                         ...
                         time_limit_per_trial=time_limit_per_trial, surrogate_type=surrogate_type,
        ...
        self.logger.info("Total space size:{}".format(estimate_size(self.config_space,
                                                                    'knobs/knobs.json')))
    ...
    def run(self):
        ...
        for _ in tqdm(range(self.iteration_id, self.max_iterations)):
            ...
            #         self.optimizer = self.select_optimizer(type=self.auto_optimizer_type,
            #                                                space=self.config_space if compact_space is None else compact_space)
            #
            ...
            #     self.logger.info("[Iteration {}] [{},{}] Total space size:{}".format(self.iteration_id, self.space_step,
            #                                                                          self.space_step_limit,
            #                                                                          estimate_size(space,
            #                                                                                        self.knob_config_file)))

            _, _, _, objs = self.iterate(compact_space)

            # determine whether explore one more step in the space
            # if (self.space_transfer or self.auto_optimizer) and len(self.history_container.get_incumbents()) > 0 and \
            #         objs[0] < self.history_container.get_incumbents()[0][1]:
    ...
    def knob_selection(self):
        ...
        if self.incremental == 'none':
            ...
            if self.incremental == 'increase':
                ...
                if not num_hps == len(
                        self.config_space.get_hyperparameter_names()) or self.iteration_id == self.init_num:
                ...
                num_hps = int(
                    self.num_hps_init * (0.6 ** incremental_step))  # - incremental_step * self.incremental_every
                ...
                if not num_hps == len(self.config_space.get_hyperparameter_names()):
                    _, self.knob_rank = self.selector.knob_selection(self.config_space_all, self.history_container,
                                                                     self.num_hps_max)
                    new_config_space = ConfigurationSpace()
                    for knob in self.knob_rank[:num_hps]:
                        new_config_space.add_hyperparameter(self.config_space_all.get_hyperparameter(knob))
    ...
    def select_optimizer(self, space, type='learned'):
        ...
        if not hasattr(self, 'rgpe_op'):
            ...
            self.rgpe_op = RGPE(self.config_space, self.history_workload_data, seed, num_src_hpo_trial=-1,
                                only_source=False)
    ...
    def evaluate(self, config):
        ...
        observation = Observation(
            ...
            trial_state=trial_state, elapsed_time=elapsed_time, iter_time=iter_time, EM=em, resource=resource, IM=im,
            info=info, context=self.current_context
        )
    ...
    def get_compact_space(self):
        ...
        for j in range(len(surrogate_list)):
            ...
            self.logger.info(
                "[{}] similarity:{} default:{}, quantile:{}, threshold:{}".format(surrogate_list[j].task_id,
                                                                                  similarity_list[j],
                                                                                  default_performance, quantile,
